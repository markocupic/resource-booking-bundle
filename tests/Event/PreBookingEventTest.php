...
class PreBookingEventTest extends TestCase
{
    public function testGettersReturnTheConstructorArguments(): void
    {
        $request = $this->createMock(Request::class);
        $ajaxResponse = new AjaxResponse('book');
        $sessionBag = $this->createMock(ArrayAttributeBag::class);
        $user = $this->createMock(UserInterface::class);
        $collection = $this->createMock(Collection::class);

        $event = new PreBookingEvent($request, $ajaxResponse, $sessionBag, $user, $collection);
    ...
    {
        $event = new PreBookingEvent(
            $this->createMock(Request::class),
            new AjaxResponse('book'),
